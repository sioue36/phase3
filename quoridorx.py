# -*- coding: utf-8 -*-
'''
Module définissant la classe QuordidorX.
'''
    ...
    def afficher(self):
        if self.automatique:
            self.window = pyglet.window.Window(
                width=500, height=500, caption='Quoridor automatique' \
                    '(Attendez que la partie se termine)')

            @self.window.event
            def on_draw():
                nom_des_joueurs = pyglet.text.Label('Blanc : {}       Noir : {}'.format(
                    self.grille['joueurs'][0]['nom'], self.grille['joueurs'][1]['nom']),
                    x=self.window.width/2, y=self.window.height-5,
                    anchor_x='center', anchor_y='top')
                murs_a_placer = pyglet.text.Label('Murs : {}        Murs : {}'.format(
                    self.grille['joueurs'][0]['murs'], self.grille['joueurs'][1]['murs']),
            ...
            self.window = pyglet.window.Window(
                width=500, height=500, caption=('Quoridor (Fermez la fenêtre' \
                'pour entrer votre prochain coup)'))
...
def draw_rectangle(largeur, hauteur, x, y):
    pyglet.graphics.draw(8, pyglet.gl.GL_LINES, ('v2i', (x, y, x, y+hauteur, x, y+hauteur,
                                                        x+largeur, y+hauteur,  x+largeur, y+hauteur, x+largeur, y, x+largeur, y, x, y)))
